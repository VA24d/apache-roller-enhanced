...
/**
 * Star / unstar a weblog.
 */
public class StarWeblogAction extends UIAction {
    ...
    public StarWeblogAction() {
        this.pageTitle = "starWeblog.title";
    }
    ...
    public String unstar() {
        try {
            ...
            UserWeblogStar existing = starMgr.getWeblogStarByUserAndWeblog(
                    getAuthenticatedUser(), weblog);
            if (existing != null) {
                starMgr.removeWeblogStar(existing);
                WebloggerFactory.getWeblogger().flush();
            }
